...
void CloudReprojectionRosNode::loadParameters()
{
    ...
    this->declare_parameter<std::string>("calib_file_path", default_calib_file.string());
    ...
    const auto calib_file = odin_ros_driver::paths::resolve_path(
        "odin_ros_driver",
        this->get_parameter("calib_file_path").as_string(),
        std::filesystem::path("config") / "calib.yaml");

    // Load camera parameters from calib.yaml file directly
    YAML::Node calib_config;
    try {
        calib_config = YAML::LoadFile(calib_file.string());
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "config: " << config_file.string() << std::endl;
    ...
    const auto calib_file = odin_ros_driver::paths::resolve_path(
        ...
        "",
        std::filesystem::path("config") / "calib.yaml");
    RCLCPP_INFO(temp_node->get_logger(), "Waiting for calib.yaml file at: %s", calib_file.string().c_str());
    
    int wait_count = 0;
    while (rclcpp::ok() && !fileExists(calib_file.string())) {
        if (wait_count % 10 == 0) {

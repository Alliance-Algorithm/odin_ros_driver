...
bool loadCalibParameters(std::shared_ptr<rclcpp::Node> node, const std::string& calib_file_path) {
    try {
        RCLCPP_INFO(
            node->get_logger(), "Loading parameters from calib.yaml file: %s",
        ...
        if (config["img_topic_0"]) {
            ...
            RCLCPP_INFO(
                node->get_logger(), "Loaded parameter: img_topic_0 = %s", img_topic.c_str());
        }
        ...
        if (config["Tcl_0"]) {
            ...
            RCLCPP_INFO(
                node->get_logger(), "Loaded parameter: Tcl_0 (transform matrix with %zu elements)",
                tcl_matrix.size());
        }
        ...
        if (config["cam_0"]) {
            ...
            if (cam_0["cam_model"]) {
                ...
                RCLCPP_INFO(
                    node->get_logger(), "Loaded parameter: cam_0.cam_model = %s",
                    cam_model.c_str());
            }
            ...
            if (cam_0["image_width"]) {
                ...
                RCLCPP_INFO(
                    node->get_logger(), "Loaded parameter: cam_0.image_width = %d", image_width);
            }
            ...
            if (cam_0["image_height"]) {
                ...
                RCLCPP_INFO(
                    node->get_logger(), "Loaded parameter: cam_0.image_height = %d", image_height);
            }

            std::vector<std::string> distortion_params = {"k2", "k3", "k4", "k5",
                                                          "k6", "k7", "p1", "p2"};
            for (const auto& param : distortion_params) {
                if (cam_0[param]) {
                    ...
                    RCLCPP_INFO(
                        node->get_logger(), "Loaded parameter: cam_0.%s = %f", param.c_str(),
                        value);
                }
            }
            ...
            if (cam_0["maxIncidentAngle"]) {
                ...
                RCLCPP_INFO(
                    node->get_logger(), "Loaded parameter: cam_0.maxIncidentAngle = %d",
                    max_incident_angle);
            }
        }
...
int main(int argc, char** argv) {
    ...
    std::string config_file =
        node->declare_parameter<std::string>("config_file", getDefaultConfigFilePath());
    ...
    if (calib_file_path.empty()) {
        RCLCPP_ERROR(
            node->get_logger(),
            "calib_file_path parameter is empty. Pass a writable generated calibration path.");
        return 1;
    }
    ...
    while (rclcpp::ok() && !fileExists(calib_file_path)) {
        RCLCPP_INFO_THROTTLE(
            node->get_logger(), *node->get_clock(), 5000, "Still waiting for calib.yaml file...");
    ...
    try {
        params_override.push_back(
            rclcpp::Parameter(
                "cam_0.image_width", node->get_parameter("cam_0.image_width").as_int()));
        params_override.push_back(
            rclcpp::Parameter(
                "cam_0.image_height", node->get_parameter("cam_0.image_height").as_int()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.A11", node->get_parameter("cam_0.A11").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.A12", node->get_parameter("cam_0.A12").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.A22", node->get_parameter("cam_0.A22").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.u0", node->get_parameter("cam_0.u0").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.v0", node->get_parameter("cam_0.v0").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k2", node->get_parameter("cam_0.k2").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k3", node->get_parameter("cam_0.k3").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k4", node->get_parameter("cam_0.k4").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k5", node->get_parameter("cam_0.k5").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k6", node->get_parameter("cam_0.k6").as_double()));
        params_override.push_back(
            rclcpp::Parameter("cam_0.k7", node->get_parameter("cam_0.k7").as_double()));
        params_override.push_back(
            rclcpp::Parameter("Tcl_0", node->get_parameter("Tcl_0").as_double_array()));
        ...
        RCLCPP_ERROR(
            node->get_logger(), "Error preparing parameters for DepthImageRos2Node: %s", e.what());
        return 1;
    }
    try {

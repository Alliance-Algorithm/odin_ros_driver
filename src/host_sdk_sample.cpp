...
static std::string g_runtime_dir = "";
static std::string g_calib_file_path = "";

static std::string get_default_runtime_directory() {
...
#ifdef ROS2
static std::string get_default_ros2_config_file() {
...
double get_ptp_smoothed_delay() { return g_ptp_delay_smooth.load(std::memory_order_relaxed); }
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "%s FPS: %f (count: %d, elapsed: %f)", name, fps,
...
public:
    void setDtofSubframeODR(int interval) override { dtof_subframe_interval_time = interval; }

    int getDtofSubframeODR() const override { return dtof_subframe_interval_time; }
    ...
    int cloudRawConfidenceThreshold() const { return cloud_raw_confidence_threshold; }
...
void collect_children(pid_t pid, std::vector<pid_t>& all) {
    std::string task_path =
        "/proc/" + std::to_string(pid) + "/task/" + std::to_string(pid) + "/children";
...
static bool convert_calib_to_cam_in_ex(
    const std::string& calib_path, const std::filesystem::path& out_path);
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("signal_handler"), "Received signal %d, shutting down...", signum);
#else
...
#ifdef ROS2
                RCLCPP_INFO(
                    rclcpp::get_logger("device_cb"), "pose_index: %d",
                    g_ros_object->get_pose_index());
                RCLCPP_INFO(
                    rclcpp::get_logger("device_cb"), "cloud_index: %d",
                    g_ros_object->get_cloud_index());
                RCLCPP_INFO(
                    rclcpp::get_logger("device_cb"), "image_index: %d",
            ...
            if (g_custom_map_mode == 1) {
                ...
                if (result == 0) {
                    ...
                    if (last_save_map_val == 1 && value == 0) {
                        ...
                        std::string map_name = g_mapping_result_file_name != ""
                                                 ? g_mapping_result_file_name
                                                 : "map_" + std::string(map_save_time) + ".bin";
#ifdef ROS2
                        RCLCPP_INFO(
                            rclcpp::get_logger("param_monitor"),
                            "Map is saved on device, now transfering to [%s/%s]", map_dir.c_str(),
                            map_name.c_str());
#else
                        ROS_INFO(
                            "Map is saved on device, now transfering to [%s/%s]", map_dir.c_str(),
...
#endif
                        int ret =
                            lidar_get_mapping_result(odinDevice, map_dir.c_str(), map_name.c_str());
                        if (ret < 0) {
#ifdef ROS2
                            RCLCPP_WARN(
                                rclcpp::get_logger("param_monitor"),
                                "Failed to get mapping result");
#else
                            ...
                            RCLCPP_INFO(
                                rclcpp::get_logger("param_monitor"),
                                "map get start success, now transfering...");
#else
                            ...
                            RCLCPP_WARN(
                                rclcpp::get_logger("param_monitor"),
                                "Failed to get mapping result, error code: %d", ret);
#else
                    ...
                    RCLCPP_INFO(
                        rclcpp::get_logger("param_monitor"),
                        "file transfering, try again later...");
#else
    ...
    if (std::getline(file, line)) {
        ...
        if (command == "set") {
            ...
            try {
                ...
                int result =
                    lidar_set_custom_parameter(odinDevice, param_name.c_str(), &value, sizeof(int));

                if (result == 0) {
#ifdef ROS2
                    RCLCPP_INFO(
                        rclcpp::get_logger("command_processor"), "Successfully set %s = %d",
...
#ifdef ROS2
                    RCLCPP_ERROR(
                        rclcpp::get_logger("command_processor"), "Failed to set %s = %d, error: %d",
                        param_name.c_str(), value, result);
#else
                    ROS_ERROR(
...
#ifdef ROS2
                RCLCPP_ERROR(
                    rclcpp::get_logger("command_processor"), "Invalid value: %s",
                    value_str.c_str());
#else
...
#ifdef ROS2
            RCLCPP_WARN(
                rclcpp::get_logger("command_processor"), "Unknown command: %s", command.c_str());
#else
...
#ifdef ROS2
            RCLCPP_ERROR(
                rclcpp::get_logger("device_cb"), "Failed to open cam_in_ex.txt for write: %s",
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "Wrote cam_in_ex.txt to: %s",
            out_path.string().c_str());
#else
...
static void imu_thread_routine() {
...
#ifdef ROS2
    RCLCPP_INFO(
        rclcpp::get_logger("imu_thread"), "IMU dedicated thread started (priority: %d)",
    ...
    while (g_imu_thread_running) {
        ...
        // Wait for IMU data
        g_imu_queue_cv.wait(lock, []() { return !g_imu_queue.empty() || !g_imu_thread_running; });
...
static void lidar_data_callback(const lidar_data_t* data, void* user_data) {
    ...
    switch (data->type) {
    case LIDAR_DT_NONE: printf("empty lidar data type: %x\n", data->type); break;
    ...
    case LIDAR_DT_SLAM_ODOMETRY:
        if (g_sendodom) {
            g_ros_object->publishOdometry(
                (capture_Image_List_t*)&data->stream, OdometryType::STANDARD, g_show_path,
                g_show_camerapose);
        }
    ...
    case LIDAR_DT_DEV_STATUS:
        ...
        if (g_devstatus_log) {
            if (dev_status_csv_file) {
                ...
                int rc = 0;
                rc = std::fprintf(
                    dev_status_csv_file, "%.2f,%d,%d,%d,%d,%d,", dev_info_data->uptime_seconds,
                    dev_info_data->soc_thermal.package_temp, dev_info_data->soc_thermal.cpu_temp,
                    dev_info_data->soc_thermal.center_temp, dev_info_data->soc_thermal.gpu_temp,
                ...
                rc = std::fprintf(
                    dev_status_csv_file, "%d,%d,", dev_info_data->dtof_sensor.tx_temp,
        ...
        if (g_show_fps) {
            ...
            printf(
                ...
                ((float)dev_info_data->rgb_sensor.configured_odr) / 1000,
                ((float)dev_info_data->rgb_sensor.tx_odr) / 1000, cal_fps(&rgb_rx_fps, "rgb_rx"));
            ...
            printf(
                ...
                ((float)dev_info_data->imu_sensor.configured_odr) / 1000,
                ((float)dev_info_data->imu_sensor.tx_odr) / 1000, cal_fps(&imu_rx_fps, "imu_rx"));
...
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("dev_info"), "Total RAM (parent + children): %.2f MB", total_mb);
#else
    ...
    case LIDAR_DT_SLAM_ODOMETRY_HIGHFREQ: {
        if (g_sendodom) {
            g_ros_object->publishOdometry(
                (capture_Image_List_t*)&data->stream, OdometryType::HIGHFREQ, false, false);
        }
    ...
    case LIDAR_DT_SLAM_ODOMETRY_TF: {
        if (g_custom_map_mode == 2) {
            g_ros_object->publishOdometry(
                (capture_Image_List_t*)&data->stream, OdometryType::TRANSFORM, false, false);
    ...
    case LIDAR_DT_NTP: {
        ...
        if (data_len == sizeof(ptp_sync_data_t)) {
            ptp_sync_data_t* ptp_data = (ptp_sync_data_t*)data->stream.imageList[0].pAddr;
            {
                ...
                for (double v : g_ptp_delay_buf)
                    delay_sum += v;
                double offset_sum = 0.0;
                for (double v : g_ptp_offset_buf)
                    offset_sum += v;
    ...
    default: printf("Unknown lidar data type: %x", data->type); return;
    }
}
...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach) {
    ...
    if (attach == true) {
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "Hardware connected, starting software connection...");
#else
...
#ifdef ROS2
            RCLCPP_FATAL(
                rclcpp::get_logger("device_cb"), "Device connected to USB 2.0 port. This device "
                                                 "requires USB 3.0 or higher. Exiting program.");
#else
            ROS_FATAL(
                "Device connected to USB 2.0 port. This device requires USB 3.0 or higher. Exiting "
                "program.");
#endif
...
#ifdef ROS2
        config_dir = std::filesystem::path(g_calib_file_path).parent_path().string();
#else
        ...
        if (config_dir.empty()) {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger("device_cb"),
                "Generated calibration directory is empty. calib_file_path=%s",
                g_calib_file_path.c_str());
#else
            ROS_ERROR(
                "Generated calibration directory is empty. calib_file_path=%s",
                g_calib_file_path.c_str());
        ...
        std::error_code config_dir_err;
        std::filesystem::create_directories(config_dir, config_dir_err);
        if (config_dir_err) {
#ifdef ROS2
            RCLCPP_ERROR(
                rclcpp::get_logger("device_cb"), "Failed to create calibration directory %s: %s",
                config_dir.c_str(), config_dir_err.message().c_str());
#else
            ROS_ERROR(
                "Failed to create calibration directory %s: %s", config_dir.c_str(),
                config_dir_err.message().c_str());
#endif
            return;
        }
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "Calibration files will be saved to: %s",
            config_dir.c_str());
#else
        ...
        std::filesystem::path per_con_log_root_dir;
        {
            ...
            std::filesystem::path base_log_dir =
                log_root_dir_.empty() ? std::filesystem::path(config_dir) : log_root_dir_;
            ...
            if (per_con_dir_err) {
                ...
                RCLCPP_WARN(
                    ...
                    per_con_log_root_dir.c_str(), per_con_dir_err.message().c_str());
#else
                ROS_WARN(
                    "Failed to create per-connection log directory %s: %s",
                    per_con_log_root_dir.c_str(), per_con_dir_err.message().c_str());
        ...
        auto elapsed =
            std::chrono::duration_cast<std::chrono::seconds>(now - software_connect_start);
        if (elapsed.count() >= 60) {
#ifdef ROS2
            RCLCPP_FATAL(
                rclcpp::get_logger("device_cb"),
        ...
        if (lidar_get_version(odinDevice, &version)) {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger("device_cb"),
                "Failed to get device firmware version, potential incompatible, please upgrade "
                "device firmware and retry.");
#else
            ROS_ERROR(
                "Failed to get device firmware version, potential incompatible, please upgrade "
                "device firmware and retry.");
            ...
            RCLCPP_INFO(
                ...
                "ros_driver_version:%s, recommended_firmware_version:%d.%d.%d", ros_driver_version,
            ...
            RCLCPP_INFO(
                rclcpp::get_logger(__func__), "kernel_version: V%d.%d.%d",
                version.kernel_version.major, version.kernel_version.minor,
                version.kernel_version.patch);
            RCLCPP_INFO(
                rclcpp::get_logger(__func__), "mcu_version: V%d.%d.%d", version.mcu_version.major,
                version.mcu_version.minor, version.mcu_version.patch);
            RCLCPP_INFO(
                rclcpp::get_logger(__func__), "soc_version: V%d.%d.%d", version.soc_version.major,
            ...
            RCLCPP_INFO(
                rclcpp::get_logger(__func__), "slam_version: V%d.%d.%d", version.slam_version.major,
            ...
            ROS_INFO(
                "kernel_version: V%d.%d.%d", version.kernel_version.major,
                version.kernel_version.minor, version.kernel_version.patch);
            ROS_INFO(
                "mcu_version: V%d.%d.%d", version.mcu_version.major, version.mcu_version.minor,
                version.mcu_version.patch);
            ROS_INFO(
                "soc_version: V%d.%d.%d", version.soc_version.major, version.soc_version.minor,
            ...
            ROS_INFO(
                "slam_version: V%d.%d.%d", version.slam_version.major, version.slam_version.minor,
                version.slam_version.patch);
#endif

            if (version.soc_version.major < required_firmware_version_major
                || (version.soc_version.minor < required_firmware_version_minor)
                || (version.soc_version.patch < required_firmware_version_patch)) {
#ifdef ROS2
                RCLCPP_ERROR(
                    rclcpp::get_logger(__func__),
                    "The soc version is too low, please upgrade the device firmware to at least "
                    "%d.%d.%d\n",
                    required_firmware_version_major, required_firmware_version_minor,
                    required_firmware_version_patch);
#else
        ...
        if (g_save_log) {
            if (lidar_enable_encrypted_device_log(
                    const_cast<lidar_device_info_t*>(device), per_con_log_root_dir.c_str())) {
#ifdef ROS2
...
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("device_cb"), "Encrypted device log enabled at: %s",
...
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("device_cb"), "Encrypted device log disabled via configuration");
#else
...
#endif
            break;
        case LIDAR_DEVICE_INITIALIZED: need_open_device = false;
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("device_cb"), "Device state: initialized, skip opening device");
#else
        ...
        case LIDAR_DEVICE_STREAMING:
            ...
            exit(1);
            break;
        case LIDAR_DEVICE_STREAM_STOPPED: need_open_device = false; get_calib_file = false;
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("device_cb"), "Device state: stream stopped, resume streaming");
#else
...
#ifdef ROS2
            RCLCPP_WARN(
                rclcpp::get_logger("device_cb"), "Unknown device initial state: %d",
        ...
        std::string calib_config = g_calib_file_path;
...
#ifdef ROS2
            RCLCPP_INFO(
                rclcpp::get_logger("device_cb"), "Successfully retrieved calibration files");
#else
...
#ifdef ROS2
                RCLCPP_ERROR(
                    rclcpp::get_logger("device_cb"), "Failed to initialize point cloud renderer");
#else
...
#ifdef ROS2
            RCLCPP_WARN(
                rclcpp::get_logger("device_cb"), "Renderer config file not found: %s",
                calib_config.c_str());
#else
...
#ifdef ROS2
            RCLCPP_WARN(
                rclcpp::get_logger("ros[host_sdk_sample]"),
...
// Log the configured frame rate
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("ros[host_sdk_sample]"), "DTOF sensor frame rate set to %.1f fps",
            g_dtof_fps / 10.0);
#else
        ...
        if (g_custom_map_mode == 1) {
            ...
            if (result == 0) {
#ifdef ROS2
                RCLCPP_INFO(
                    rclcpp::get_logger("command_processor"), "Successfully initialized %s = %d",
                    "save_map", save_map_init_value);
#else
                ...
                RCLCPP_ERROR(
                    rclcpp::get_logger("command_processor"),
                    "Failed to initialize %s = %d, error: %d", "save_map", save_map_init_value,
                    result);
#else
                ROS_ERROR(
                    "Failed to initialize %s = %d, error: %d", "save_map", save_map_init_value,
            ...
            if (g_relocalization_map_abs_path != ""
                ...
                && lidar_set_relocalization_map(odinDevice, g_relocalization_map_abs_path.c_str())
                       == 0) {
#ifdef ROS2
        ...
        if (g_send_image_mask == 1) {
            if (g_image_mask_abs_path != "" && std::filesystem::exists(g_image_mask_abs_path)) {
...
#ifdef ROS2
                    RCLCPP_INFO(
                        rclcpp::get_logger("device_cb"), "Image mask set successfully: %s",
...
#ifdef ROS2
                    RCLCPP_ERROR(
                        rclcpp::get_logger("device_cb"), "Failed to set image mask: %s, error: %d",
                        g_image_mask_abs_path.c_str(), ret);
#else
                    ROS_ERROR(
                        "Failed to set image mask: %s, error: %d", g_image_mask_abs_path.c_str(),
                        ret);
#endif
                }
            } else {
                ...
                RCLCPP_WARN(
                    rclcpp::get_logger("device_cb"),
                    "Image mask path not set or file not found: %s", g_image_mask_abs_path.c_str());
#else
                ROS_WARN(
                    "Image mask path not set or file not found: %s", g_image_mask_abs_path.c_str());
#endif
        ...
        if (g_reset_algo == 1) {
            int reset_value = 1;
            int ret =
                lidar_set_custom_parameter(odinDevice, "algo_reset", &reset_value, sizeof(int));
            if (ret == 0) {
#ifdef ROS2
                RCLCPP_INFO(
                    rclcpp::get_logger("device_cb"), "Algo reset command sent successfully");
#else
...
#ifdef ROS2
                RCLCPP_ERROR(
                    rclcpp::get_logger("device_cb"), "Failed to send algo reset command, error: %d",
                    ret);
#else
        ...
        if (!dev_status_csv_file) {
            ...
            const char* header = "uptime_seconds,package_temp,cpu_temp,center_temp,gpu_temp,npu_"
                                 "temp,dtof_tx_temp,dtof_rx_temp,"
                                 "cpu0,cpu1,cpu2,cpu3,cpu4,cpu5,cpu6,cpu7,ram_use(%),"
                                 "rgb_configured_odr,rgb_tx_odr,rgb_rx_odr,dtof_configured_odr,"
                                 "dtof_tx_odr,dtof_rx_odr,imu_configured_odr,imu_tx_odr,imu_rx_odr,"
                                 "slam_cloud_tx_odr,slam_cloud_rx_odr,slam_odom_tx_odr,slam_odom_"
                                 "rx_odr,slam_odom_highfreq_tx_odr,slam_odom_highfreq_rx_odr,"
                                 "host_ram_use(mb)\n";
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "Software connection successful in %ld seconds",
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::string config_file =
            node->declare_parameter<std::string>("config_file", get_default_ros2_config_file());
        g_runtime_dir =
            node->declare_parameter<std::string>("runtime_dir", get_default_runtime_directory());
        g_calib_file_path = node->declare_parameter<std::string>(
            "calib_file_path", (std::filesystem::path(g_runtime_dir) / "calib.yaml").string());
#else
        ...
        if (g_runtime_dir.empty()) {
            throw std::runtime_error("runtime_dir parameter is empty");
        }

        if (g_calib_file_path.empty()) {
            throw std::runtime_error("calib_file_path parameter is empty");
        }

        std::filesystem::create_directories(g_runtime_dir);
        std::filesystem::create_directories(std::filesystem::path(g_calib_file_path).parent_path());
#endif
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("init"), "Command file path set to: %s",
            g_command_file_path.c_str());
#else
        ...
        g_dtof_fps = get_key_value(
            "dtof_fps", 145); // Read DTOF frame rate from config (100=10fps, 145=14.5fps)
        ...
        auto get_key_str_value = [&](const std::string& key,
        ...
        std::string map_dir = "";
#ifdef ROS2
        data_dir = (std::filesystem::path(g_runtime_dir) / "recorddata").string();
        log_dir = (std::filesystem::path(g_runtime_dir) / "log").string();
        map_dir = (std::filesystem::path(g_runtime_dir) / "map").string();
#else
...
#ifdef ROS2
            RCLCPP_INFO(
                node->get_logger(), "Enabling SDK IMU smooth sending at %d Hz",
...
#ifdef ROS2
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "pose_index: %d", g_ros_object->get_pose_index());
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "cloud_index: %d", g_ros_object->get_cloud_index());
        RCLCPP_INFO(
            rclcpp::get_logger("device_cb"), "image_index: %d", g_ros_object->get_image_index());
#else
        ROS_INFO("pose_index: %d", g_ros_object->get_pose_index());
        ROS_INFO("cloud_index: %d", g_ros_object->get_cloud_index());
        ROS_INFO("image_index: %d", g_ros_object->get_image_index());
